...
{
    public partial class NVPHUCAP : Form
    {
        public NVPHUCAP()
        {
            InitializeComponent();
        ...
        private void NVPHUCAP_Load(object sender, EventArgs e)
        {
            // TODO: This line of code loads data into the 'quanLyNhanVienv2DataSet2.NHANVIENPC' table. You can move, or remove it, as needed.
            // this.nHANVIENPCTableAdapter.Fill(this.quanLyNhanVienv2DataSet2.NHANVIENPC);
            string sql = "select * from nhanvienpc";
        ...
        private void dataGridView1_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            tb_mapc.Text = dataGridView1.Rows[i].Cells[1].Value?.ToString();
        ...
        private void button1_Click(object sender, EventArgs e)// them
        {
            try
            {
                ...
                if (string.IsNullOrWhiteSpace(tb_mapc.Text) || string.IsNullOrWhiteSpace(tb_manv.Text)
                ...
                using (SqlConnection connection = SqlConnectionData.connect())
                {
                    ...
                    SqlCommand checkCommand = new SqlCommand("SELECT COUNT(*) FROM nhanvienpc WHERE mapc = @mapc", connection);
                    ...
                    command.CommandText = "INSERT INTO nhanvienpc VALUES( @manv, @mapc,@tienpc)";
        ...
        private void button2_Click(object sender, EventArgs e)//ssua
        {
            try
            {
                ...
                using (SqlConnection connection = SqlConnectionData.connect())
                {
                    ...
                    SqlCommand command = connection.CreateCommand();
                    command.CommandText = "UPDATE nhanvienpc SET manv = @manv, tienpc = @tienpc WHERE mapc = @mapc";
                    command.Parameters.AddWithValue("@mapc", tb_mapc.Text);
                    command.Parameters.AddWithValue("@manv", tb_manv.Text);
                    command.Parameters.AddWithValue("@tienpc", tb_tienpc.Text);
            ...
            try
            {
                ...
                using (SqlConnection connection = SqlConnectionData.connect())
                {
                    ...
                    if (result == DialogResult.Yes)
                    {
                        ...
                        SqlCommand command = connection.CreateCommand();
                        command.CommandText = "DELETE FROM nhanvienpc WHERE mapc = @mapc";
                        command.Parameters.AddWithValue("@mapc", tb_mapc.Text);
            ...
            try
            {
                using (SqlConnection connection = SqlConnectionData.connect())
                {
                    ...
                    if (count > 0)
                    {
                        ...
                        SqlCommand selectCommand = new SqlCommand("SELECT * FROM nhanvienpc WHERE mapc = @mapc", connection);
